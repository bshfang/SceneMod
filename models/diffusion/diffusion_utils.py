...
from utils.utils import PlaceHolder
...
def linear_beta_schedule(timesteps, b_start=0.0001, b_end=0.02):
    """
    linear schedule
    as proposed in https://openreview.net/forum?id=-NEXDKk8gZ
    """

    betas = np.linspace(b_start, b_end, timesteps+1)
    alphas = 1. - betas
    alphas_cumprod = np.cumprod(alphas, axis=0)
...
def sample_feature_noise(X_size, E_size, y_size, node_mask, undirected_E=True, use_node_mask=True):
    ...
    if use_node_mask:
        ...
        return PlaceHolder(X=epsX, E=epsE, y=epsy)
